...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        xs = orm.QuestionDataType.objects.all().filter(title='Date')
        if len(xs):
            x = xs[0]
            x.db_type = 'db.models.DateField(null=True, blank=True)'
            x.save()
    ...
    def backwards(self, orm):
        ...
        if len(xs):
            x = xs[0]
            x.db_type = 'django.db.models.DateField(null=True, blank=True)'
            x.save()
